...
function extractAssetCode(rawText) {
    const CT = rawText ? rawText.trim() : "";
    let extractedText = "";
    let cutMethodMessage = "";

    if (CT.length < 30) {
        extractedText = CT.substring(0, 22).trim();
        cutMethodMessage = "(ตัดตำแหน่ง 1-22)";
    } else {
        extractedText = CT.substring(8, 30).trim();
...
function onScanSuccess(decodedText, decodedResult) {
    ...
    const CT = decodedText ? decodedText.trim() : "";

    if (CT === "") {
        playBeepSound('error');
        allResultElement.innerText = "SBN";
        splitElement.innerHTML = `<span style="color: #ef4444; font-weight: 700;">❌ ค่าที่สแกนได้เป็นค่าว่าง</span>`;
        handleProcessingEnd(1500);
        return;
    }

    const { extractedText, cutMethodMessage } = extractAssetCode(CT);
    const digitCount = extractedText.length;

    if (digitCount !== 22) {
        playBeepSound('error');
        allResultElement.innerText = CT;
        splitElement.innerHTML = `<span style="color: #ef4444; font-weight: 700;">❌ ไม่ใช่รหัสครุภัณฑ์ </span>`/* ${cutMethodMessage} (${digitCount}/22)</span>`;*/
        handleProcessingEnd(1500);
        return;
    }
...
function clearHistory() {
    ...
    if (confirmClear) {
        scanHistoryList = [];
        updateHistoryUI();
        document.getElementById('result-all').innerText = "WTS";
